...
class IndexWindow(QMainWindow, Ui_IndexWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.core_buttons = [self.btn_import,
                             self.btn_save, self.btn_save_as, self.btn_run]
    ...
    def check_registered(self, mac_address):
        result = plugins.check_registered(mac_address)
        print(result)
        if result['code'] == 1 or result['code'] == 2:
            QMessageBox.warning(
                self, 'Warning', '您的软件尚未授权，请尽快注册！', QMessageBox.Yes)
            self.setWindowTitle('AutoConfiguration(未授权)')
            for btn in self.core_buttons:
                btn.setEnabled(False)
        elif result['code'] == -1:
            QMessageBox.warning(
                self, 'Warning', '您的软件配置文件错误，请删除cfg.ini后重新运行！', QMessageBox.Yes)
            exit(1)
        else:
            QMessageBox.information(
                self, 'Welcome', '您的软件已授权，请尽情使用！', QMessageBox.Yes)
    ...
    def on_btn_save_clicked(self):
        ...
        for i in range(len(self.config_list)):
            cconfig = self.config_list[i][1]() if i == 0 else self.config_list[i][1](
                self.config_list[i - 1][-1])
            cconfig_file = self.config_list[i][2]()
            cconfig_ui = self.config_list[i][3]()

            config_file_name = os.path.join(
                path_configs, self.config_list[i][0])
    ...
    def on_btn_save_as_clicked(self):
        if len(self.config_list) == 0:
            QMessageBox.warning(
                self, '消息', '无可供导出的配置文件，请先尝试导入', QMessageBox.Yes)
            return
        file_name, file_type = QFileDialog.getSaveFileName(
            self, '文件保存', self.cwd, 'Text Files(*.txt)')
        if file_name == '':
            return
        else:
            path_configs = os.path.join(os.path.dirname(file_name), 'params')
    ...
    def on_btn_import_clicked(self):
        file_name, file_type = QFileDialog.getOpenFileName(
            self, 'Choose the file', self.cwd, 'Text Files(*.txt)')
        ...
        if not os.path.exists(path_configs):
            QMessageBox.warning(
                self, 'Warning', '无可供导入的配置文件，请先尝试重新导入', QMessageBox.Yes)
            return
        name_configs = os.listdir(path_configs)
        self.config_list = list(
            filter(lambda c: c[0] in name_configs, self.config_list))
        ...
        QMessageBox.information(
            self, 'Success', 'Config File Load Success!', QMessageBox.Yes)
        return
    ...
    def chek_pannels(self):
        ...
        num_error += self.check_row(self.tbl_cfg1,
                                    0, item_cfg1_TM, r'(raw)|(ms1)|(mgf)')
        item_num = self.list_cfg1_PM.count()
        item_cfg1_PM = '|'.join(
            [self.list_cfg1_PM.item(i).text() for i in range(item_num)])
    ...
    def on_list_cfg2_selected_doubleClicked(self, index):
        ...
        if len(select_items) > 1:
            ...
            self.list_cfg2_selected.takeItem(
                self.list_cfg2_selected.row(*select_items))
    ...
    def on_list_cfg2_unselect_doubleClicked(self, index):
        ...
        if len(select_items) > 1:
            ...
            self.list_cfg2_unselect.takeItem(
                self.list_cfg2_unselect.row(*select_items))
    ...
    def on_btn_cfg1_PM_add_clicked(self):
        file_names, file_type = QFileDialog.getOpenFileNames(
            self, 'Files select', self.cwd, 'All Type Files(*)')
        if len(file_names) == 0:
            return
    ...
    def on_btn_cfg1_PRE_clicked(self):
        dir_choose = QFileDialog.getExistingDirectory(
            self, 'Directory Select', self.cwd)
        if dir_choose == '':
            return
        else:
    ...
    def on_btn_cfg1_PF_clicked(self):
        file_name, file_type = QFileDialog.getOpenFileName(
            self, 'PATH_FASTA File Select', self.cwd, 'All Type Files(*)')
        if not os.path.exists(file_name) or file_name == '':
            return
